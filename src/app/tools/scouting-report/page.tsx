    ...
    <ToolShell tool={tool}>
      ...
      {!player ? (
        <Panel className="flex min-h-[300px] flex-col items-center justify-center text-center">
          <ClipboardList size={40} className="mb-4 text-gold" />
          ...
          <Panel>
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Badge color={GOLD}>{report.role}</Badge>
                <Badge color="#7E8CA0">Comp: {report.comp}</Badge>
              </div>
            </div>
          </Panel>
            ...
            <Panel title="Skill grades">
              ...
              <div className="mt-4 grid grid-cols-2 gap-2">
                {report.grades.map((g) => (
                  <div
                    key={g.label}
                    className="flex items-center justify-between rounded-none bg-white/[0.03] px-2.5 py-1.5"
                  >
                    <span className="text-[11px] text-white/55">{g.label}</span>
                    <span className="stat-num text-xs font-bold" style={{ color: GOLD }}>
                      {Math.round(g.value)}
                    </span>
                  </div>
                ))}
                ...
                <ul className="space-y-2.5">
                  {report.strengths.map((s) => (
                    <li key={s} className="flex items-start gap-2.5 text-sm text-white/75">
                      <span className="mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-mint/15">
                ...
                <ul className="space-y-2.5">
                  {report.weaknesses.map((w) => (
                    <li key={w} className="flex items-start gap-2.5 text-sm text-white/75">
                      <span className="mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-rose/15">
            ...
            <ol className="space-y-2.5">
              {report.priorities.map((p, i) => (
                <li key={p} className="flex items-start gap-3 text-sm text-white/75">
                  <span
                    className="stat-num flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold"
